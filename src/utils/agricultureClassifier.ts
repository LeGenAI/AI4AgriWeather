/**
 * Agriculture Document Classifier
 * 탄자니아 및 동아프리카 농업 특성을 반영한 문서 자동 분류 시스템
 */
...
/**
 * 문서를 분석하여 농업 관련 분류 정보를 생성합니다.
 * @param title 문서 제목
 * @param content 문서 내용
 * @returns 분류 결과
 */
export function classifyDocument(title: string, content: string): ClassificationResult {
...
export function summarizeClassification(result: ClassificationResult): string {
  ...
  if (result.crops.length > 0) {
    parts.push(`Crops: ${result.crops.join(', ')}`);
  }
  
  if (result.seasons.length > 0) {
    parts.push(`Seasons: ${result.seasons.join(', ')}`);
  }
  
  if (result.activities.length > 0) {
    parts.push(`Activities: ${result.activities.join(', ')}`);
  }
  
  if (result.regions.length > 0) {
    parts.push(`Regions: ${result.regions.join(', ')}`);
  }
